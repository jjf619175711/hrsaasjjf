<template>
  ...
  <div>
    ...
    <el-row type="flex" justify="end">
      <!-- year 使用组件给定一个日期 日期取年，年取前五年 + 后五年 -->
      <el-select v-model="currentYear" size="small" style="width:120px" @change="dataChange">
        <el-option v-for="item in yearList" :key="item" :label="item" :value="item" />
      </el-select>
      <!-- month -->
      <el-select v-model="currentMonth" size="small" style="width:120px;margin-left:12px" @change="dataChange">
...
export default {
  ...
  created() {
    ...
    this.currentMonth = this.startDate.getMonth() + 1 // 得到当前的月份份
    // 快速生成数组的方法
